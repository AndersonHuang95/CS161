...
(defun generate-edge-clauses (e k)
    (let ((list1 (at-least-one-color (car e) 1 k)) (list2 (at-least-one-color (cadr e) 1 k)))
        (ge-helper list1 list2)
    )
)

(defun ge-helper (list1 list2)
    (cond
        ((null list1) '())
        (t (cons (list (* -1 (car list1)) (* -1 (car list2))) (ge-helper (cdr list1) (cdr list2))))
    )
)
